...
static int __alloc_block(struct vsfs_inode *inode_reg,
  ...
  read_spdk(inode_reg, sb_cached->sbi.ofs_iregion + (inode_nr / 16), 1,
            IO_QUEUE);
...
static void init_base_ptr() {
  tmp_base_ptr =
      (struct vsfs_data_block *)alloc_dma_buffer(VSFS_BLOCK_SIZE * 9);
  // at_tmp_base(sb, 0);
  // read_spdk(sb, 0, 1, IO_QUEUE);
  at_tmp_base(tmp_sb, 0);
  at_tmp_base(tmp_inode_reg, 1);
  at_tmp_base(tmp_data_reg, 2);
  at_tmp_base(tmp_dir_reg, 3);
  at_tmp_base(ptr_reg, 4);
  at_tmp_base(l3_ptr_reg, 5);
  at_tmp_base(inode_reg, 6);
  at_tmp_base(data_reg, 7);
  at_tmp_base(dma_buf, 8);
}
...
static int vsfs_creat(const char *file_name, uint32_t block_num) {
  ...
  tmp_sb_cached = (struct vsfs_cached_data *)shm_oandm(
  ...
  read_spdk(tmp_inode_reg, tmp_sb_cached->sbi.ofs_iregion, 1, IO_QUEUE);
  // tmp_dir_reg =
  ...
  for (uint32_t i = 0; i < tmp_inode_reg->entry; i++) {
    if (i % 16 == 0)
      read_spdk(tmp_dir_reg,
                tmp_sb_cached->sbi.ofs_dregion + tmp_inode_reg->l1[i / 16], 1,
                IO_QUEUE);
  ...
  f_inode = get_free_inode(tmp_sb_cached);
  f_dblock = tmp_inode_reg->entry / 16;
  d_entry = tmp_inode_reg->entry - f_dblock * 16;
...
static int vsfs_stat(char *pathname, file_stat_t *fre) {
  ...
  //     (struct vsfs_inode *)((char *)tmp_sb +
  //                           tmp_sb->info.ofs_iregion * VSFS_BLOCK_SIZE);
  read_spdk(tmp_inode_reg, tmp_sb->info.ofs_iregion, 1, IO_QUEUE);
  // tmp_data_reg =
  //     (struct vsfs_dir_block *)((char *)tmp_sb +
  //                               tmp_sb->info.ofs_dregion * VSFS_BLOCK_SIZE);
  read_spdk(tmp_data_reg, tmp_sb->info.ofs_dregion, 1, IO_QUEUE);

  if (SHOW_PROC)
    printf("vsfs_stat(): finding the file in shm\n");
  for (uint32_t i = 0; i < tmp_inode_reg->entry; i++) {
    int offset = tmp_inode_reg->l1[i / 16];
    read_spdk(tmp_data_reg, tmp_sb->info.ofs_dregion + offset, 1, IO_QUEUE);
...
static void __attribute__((unused)) vsfs_print_block_nbr(int fildes) {
  ...
  read_spdk(inode_reg, sb_cached->sbi.ofs_iregion + (target_op->inode_nr / 16), 1,
            IO_QUEUE);
  ...
  for (int i = 0; blks > 0 && i < 5; i++) {
    ...
    read_spdk(ptr_reg,
              sb_cached->sbi.ofs_dregion + inode_reg[target_op->inode_nr % 16].l2[i],
              1, IO_QUEUE);
  ...
  read_spdk(l3_ptr_reg,
            sb_cached->sbi.ofs_dregion + inode_reg[target_op->inode_nr % 16].l3[0], 1,
            IO_QUEUE);
...
static int __attribute__((unused))
__return_block(struct vsfs_inode *inode_reg, struct vsfs_data_block *data_reg,
...
static int vsfs_get_indoe_info(file_t *file, uint16_t inode_num) {
  ...
  *file = (file_t){
  ...
  if (inode_reg[inode_num % 16].size == 0 &&
    ...
    file->size = inode_reg[inode_num % 16].size;
